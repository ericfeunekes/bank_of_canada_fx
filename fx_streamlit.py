...
r = get(
    "https://www.bankofcanada.ca/valet/observations/group/FX_RATES_DAILY/json?start_date=2015-01-03"
)
...
min_date = df.d.min()
max_date = df.d.max()
resampling = {"Year": "Y", "Month": "M", "Quarter": "Q", "Week": "W", "Day": "D"}
# %%
st.title("Bank of Canada data")
...
sample = st.sidebar.selectbox("Group by", options=list(resampling.keys()), index=2)
...
st.dataframe(date_df.loc[start:end, option].resample(sample_rate).mean(), height=2000)
...
def download_link(object_to_download, download_filename, download_link_text):
    ...
    if isinstance(object_to_download, pd.DataFrame):
        object_to_download = object_to_download.to_csv(index=True, index_label="Date")
...
if st.sidebar.button("Download data as CSV"):
    tmp_download_link = download_link(
        date_df.loc[start:end, option].resample(sample_rate).mean(),
        "bocRate.csv",
